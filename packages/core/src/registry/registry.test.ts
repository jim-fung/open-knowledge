...
describe('createRegistry', () => {
  test('returns the 18 canonical + 9 compat descriptors + wildcard', () => {
    // 18 canonicals (Callout, Image, Video, Audio, Accordion, Toggle, Math,
    // MermaidFence, WiremdFence, PlotFence, Pdf, File, Tabs, Tab, Embed,
    // Mirror, MirrorSource, HtmlAlignBlock — the `<div align>` HTML form is
...
describe('builtInComponents manifest', () => {
  test('contains 18 canonical + 9 compat entries (5-pack + Math + MermaidFence + WiremdFence + PlotFence + Pdf + File + Tabs + Tab + Embed + Mirror + MirrorSource + HtmlAlignBlock + Toggle canonicals; source-form preservation + math syntax + wiki-embed compats; Mermaid + wiremd + plot are fence-only)', () => {
    expect(builtInComponents.length).toBe(27);
