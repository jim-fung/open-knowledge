  ...
  if (/\[([^\]]+)\]/.test(content)) {
    ...
    while ((match = buttonPattern.exec(content)) !== null) {
      ...
      if (isSelectText(text)) {
        ...
        slots.push({ type: "select", props, options: [] });
        continue;
      }
      if (isInputText(text)) {
        const placeholderMatch = text.match(/^([^_*]+)_{3,}$/);
      ...
      if (/:([a-z-]+):/.test(text)) {
        ...
        slots.push({ type: "button", content: "", children, props });
      } else {
        slots.push({ type: "button", content: text, props });
      }
    }
    const elements = slots.filter((s) => s !== null);
      ...
      if (!remainingText && elements.length > 1) {
        ...
        buttonMatches.forEach((match2, idx) => {
          ...
          if (element) {
            ...
            const literal = textBefore + match2[0];
            if (literal.trim()) {
...
function openerFromMatch(match) {
  return {
...
function reconstructOpenerLine(node) {
...
function stripFoldedCloserInParagraph(para) {
  ...
  const [stripped, count] = stripTrailingClosers(lastInline.value);
...
function stripFoldedCloserInBlock(node) {
  var _a, _b;
  if (node.type === "paragraph" && ((_a = node.children) == null ? void 0 : _a.length)) {
    return stripFoldedCloserInParagraph(node);
  }
  if (node.type === "list" && ((_b = node.children) == null ? void 0 : _b.length)) {
    ...
    return itemChildren.length ? { node: { ...node, children: itemChildren }, count: inner.count, closed: true } : { node: null, count: inner.count, closed: true };
  }
  return { node, count: 0, closed: false };
}
function stripFoldedCloserInListItem(item) {
  var _a;
  if (!((_a = item.children) == null ? void 0 : _a.length)) return { node: item, count: 0, closed: false };
  ...
  return children.length ? { node: { ...item, children }, count: inner.count, closed: true } : { node: null, count: inner.count, closed: true };
}
...
function extractAfterOpenerChildren(node) {
  const children = node.children ?? [];
  if (children.length === 1 && children[0].type === "text") {
    const rest2 = children[0].value.split("\n").slice(1).join("\n").trim();
    return rest2 ? [{ type: "text", value: rest2 }] : null;
  }
