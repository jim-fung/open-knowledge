 ...
 */
export type SupportedType = 'button' | 'input' | 'textarea' | 'select' | 'checkbox' | 'radio' | 'radio-group' | 'icon' | 'badge' | 'container' | 'nav' | 'nav-item' | 'brand' | 'grid' | 'grid-item' | 'row' | 'heading' | 'paragraph' | 'text' | 'image' | 'link' | 'list' | 'list-item' | 'table' | 'table-header' | 'table-row' | 'table-cell' | 'blockquote' | 'code' | 'separator' | 'tabs' | 'tab' | 'breadcrumbs' | 'demo' | 'toast' | 'skeleton' | 'spinner' | 'kbd' | 'progress' | 'meter' | 'dialog' | 'alert-dialog' | 'sheet' | 'drawer' | 'popover' | 'tooltip' | 'preview-card' | 'pagination' | 'segmented-control' | 'scroll-area' | 'sidebar' | 'menubar' | 'form' | 'field' | 'fieldset' | 'label' | 'input-group' | 'otp-field' | 'number-field' | 'autocomplete' | 'combobox' | 'command' | 'checkbox-group' | 'toggle-group' | 'switch' | 'slider' | 'toggle' | 'avatar' | 'frame' | 'group' | 'empty' | 'calendar' | 'date-picker';
/**
 ...
 */
export type NodeEmitter<N extends WiremdNode> = (node: N, format: CodegenFormat, recurse: CodegenRecurse) => string;
